...
function grensBannerHTML(info){
  ...
  return '<div class="grensbanner" role="status" aria-live="polite">' +
    '<div class="grensbanner-kop">' + flagHTML(c) + '<b>' + esc(i18n("onderweg.nuIn", { land: c.name })) + '</b>' +
...
function journeyOpPositie(pos){
  ...
  if(code === HUIDIG_LAND) return;
  HUIDIG_LAND = code;
  if(!eersteFix) meldGrenswissel(code);
  else if(VIEW === "onderweg") renderOnderweg();
}
